...
)
{
	...
	for (j = 0; j < MAX_POISONS; j++)
	{
		...
		if (((LPOBJECT)((LPOBJECT)gpGlobals->g.rgObject))[w].poison.wPoisonLevel <= wMaxLevel)
		{
...
//#endif
	...
	g_Battle.rgEnemy[wEnemyIndex].rgPoisons[j].wPoisonScript = ((LPOBJECT)gpGlobals->g.rgObject)[wPoisonID].poison.wEnemyScript;
...
)
{
	...
	WORD w = ((LPOBJECT)gpGlobals->g.rgObject)[wObjectID].enemy.wResistanceToSorcery * 10;

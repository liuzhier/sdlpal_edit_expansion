...
VOID CPalBaseIO::PAL_FreeEventObjectSprites(
	VOID
)
/*++
  Purpose:
  ...
  --*/
{
	...
	if (gpResources && gpResources->lppEventObjectSprites != NULL)
	{
...
VOID CPalBaseIO::PAL_FreePlayerSprites(
	VOID
)
/*++
  Purpose:
...
VOID CPalBaseIO::PAL_InitResources(
	VOID
)
/*++
  Purpose:
...
VOID CPalBaseIO::PAL_FreeResources(
	VOID
)
/*++
  Purpose:
...
VOID CPalBaseIO::PAL_SetLoadFlags(
	BYTE       bFlags
)
/*++
  Purpose:    Set flags to load resources.
  Parameters:    [IN]  bFlags - flags to be set.
  Return value:    None.
  --*/
{
	if (gpResources == NULL)
...
VOID CPalBaseIO::PAL_LoadResources(
	VOID
)
/*++
  Purpose:
...
--*/
{
	...
	if (gpResources->bLoadFlags & kLoadScene)
	{
		...
		gpGlobals->partyoffset = PAL_XY(0.5 * g_iTextureWidth, 0.56 * g_iTextureHeight);
#else
...
LPPALMAP CPalBaseIO::PAL_GetCurrentMap(
	VOID
)
/*++
  Purpose:
...
LPSPRITE CPalBaseIO::PAL_GetPlayerSprite(
	BYTE      bPlayerIndex
)
/*++
  Purpose:
...
LPSPRITE CPalBaseIO::PAL_GetEventObjectSprite(
	WORD      wEventObjectID
)
/*++
  Purpose:
...
WORD CPalBaseIO::PAL_SpriteGetNumFrames(
	LPCSPRITE       lpSprite
)
/*++
  Purpose:

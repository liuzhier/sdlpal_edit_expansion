...
#define UTIL_LogOutput(int ,LPCSTR,...)
...
void CAviPlay::PAL_RenderAVIFrameToSurface(	SDL_Surface* lpSurface,	const RIFFChunk* lpChunk)
{
...
BOOL CAviPlay::PAL_PlayAVI(
	LPCSTR     lpszPath
)
{
	FILE* fp = mpal->UTIL_OpenFile(lpszPath);
	if (fp == NULL)
	{
		UTIL_LogOutput(LOGLEVEL_WARNING, "Cannot open AVI file: %s!\n", lpszPath);
		return FALSE;
	}
	if (!mpal->AUDIO_GetDeviceSpec() || mpal->AUDIO_GetDeviceSpec()->channels == 0)
		return FALSE;
	...
	uint32_t   dwCurrentTime = SDL_GetTicks_New();
	uint32_t   dwNextFrameTime;
	uint32_t   dwFrameStartTime = dwCurrentTime;

	while (!fEndPlay )
	{
		if (mpal->PalQuit) fEndPlay = TRUE;
		...
		case AVI_00db:
			...
			//mpal->VIDEO_UpdateScreen(nullptr, avi->surface);
			mpal->RenderBlendCopy(mpal->gpTextureReal, avi->surface);
			mpal->VIDEO_UpdateScreen(mpal->gpTextureReal);
			dwCurrentTime = SDL_GetTicks_New();

			// Check input states here
			mpal->PAL_Delay(dwCurrentTime >= dwNextFrameTime ? 1 : dwNextFrameTime - dwCurrentTime);
			dwFrameStartTime = SDL_GetTicks_New();

			if (mpal->g_InputState.dwKeyPress & (kKeyMenu | kKeySearch))
			{
				fEndPlay = TRUE;
			}
			break;
	...
	while (chunk == NULL && pos < endPos)
	{
		...
		case AVI_00dc:
			...
			if (userbuf && buflen >= sizeof(RIFFChunkHeader) + hdr.length)
				...
				chunk = (RIFFChunk*)malloc(sizeof(RIFFChunkHeader) + hdr.length * (hdr.type == AVI_01wb ? mult : 1));
...
void*
CAviPlay::AVI_GetPlayState(void
)
{
	return &gAVIPlayState;
}

VOID CAviPlay::setPAL(class CPalBaseIO* p)
{
	mpal = p;
}
	...
	)
{
	...
	WAVEFormatEx wfe{0};
	uint32_t   block_type[MAX_AVI_BLOCK_LEVELS]{0};
	long       next_pos[MAX_AVI_BLOCK_LEVELS]{0};
	...
	while (!feof(fp) && current_level > 0)
	{
		...
		case AVI_movi:
			...
			SDL_BuildAudioCVT(&avi->cvt,
				...
				AUDIO_S16SYS,
				mpal->AUDIO_GetDeviceSpec()->channels,
				mpal->AUDIO_GetDeviceSpec()->freq);
			//
			...
			if (avi->dwBufferSize > 0)
				avi->pChunkBuffer = (uint8_t*)malloc(avi->dwBufferSize);
			else
				avi->pChunkBuffer = NULL;
			//
			...
			avi->pbAudioBuf = (uint8_t*)malloc(avi->dwAudBufLen);
...
void CAviPlay::PAL_AVIInit(void)
{
	gAVIPlayState.selfMutex = SDL_CreateMutex();
}

void CAviPlay::PAL_AVIShutdown(void)
{
